...
t_bool	we_paths_new(t_path *p, t_c8 *level)
{
	if (!wx_path_new3(&p[we_map], "data/maps/", level, "/map.txt")
		...
		|| !wx_path_new3(&p[we_sky_north],
			"data/maps/", level, "/sky_north.xpm")
		|| !wx_path_new3(&p[we_sky_east], "data/maps/", level, "/sky_east.xpm")
		|| !wx_path_new3(&p[we_sky_south],
			"data/maps/", level, "/sky_south.xpm")
		|| !wx_path_new3(&p[we_sky_west], "data/maps/", level, "/sky_west.xpm")
