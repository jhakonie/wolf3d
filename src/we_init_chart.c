...
/*   Updated: 2021/04/10 16:39:49 by jhakonie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
** todo: update zz buf to chart on line 40 allowed numbers/chars
*/
...
static t_bool	zz_load_file_to_chart(t_map *m)
{
	t_s32		fd;
	t_s32		ret;
	char		buf[WE_LOAD_FILE_TO_CHART_BUFFER_SIZE];
	...
	ret = read(fd, buf, WE_LOAD_FILE_TO_CHART_BUFFER_SIZE);
	if (ret < 0)
	{
		write(1, "read failed\n", 13);
		return (wx_false);
	}
	if (ret != WE_LOAD_FILE_TO_CHART_BUFFER_SIZE && ret > 0)
	{
