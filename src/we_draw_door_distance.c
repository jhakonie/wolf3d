...
void	we_draw_door_distance(t_ray *ray)
{
	...
	{
		ray->tile.hit.x -= fabsf(0.5f * WE_BLOCK_W);
		ray->tile.hit.y = fabsf(ray->tile.hit.x * ray->k + ray->b);
	}
	if (ray->tile.hit.x < 0 || ray->tile.hit.x > WE_MAP_SIZE)
		ray->tile.hit.x = -1;
	if (ray->tile.hit.y < 0 || ray->tile.hit.y > WE_MAP_SIZE)
		ray->tile.hit.y = -1;
}
