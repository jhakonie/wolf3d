...
/*   Updated: 2021/06/09 20:17:25 by jhakonie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	we_draw_button(t_tool *t, t_frame_buffer *fb)
{
	...
	else if (t->pre_selected && !t->selected)
	{
		we_draw_rec_texture(t->button.start, t->button.end, fb,
			&t->button.icon);
		we_draw_rec_frame(t->button.start, t->button.end, fb, 0x00FF00);
	}
	else if (t->selected)
	{
		we_draw_rec_texture(t->button.start, t->button.end, fb,
			&t->button.icon);
		we_draw_rec_frame(t->button.start, t->button.end, fb, 0xFF0000);
	}
}
